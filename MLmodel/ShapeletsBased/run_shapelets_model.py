...
def store_csv_with_repo_list(repo_list, filename):
    X_test1, X_test2, X_test3, X_test4 = [], [], [], []

    read_path = f'{result_path}segment2\\'
    df_list = []
    for repo in repo_list:
        if repo not in problem_repo:
            df = pd.read_csv(f'{read_path}{repo.replace("/", "_")}.csv',usecols=[1,21,47,63,79])
            df.fillna(0,inplace=True)

            if len(df)<105:
                lst1,lst2,lst3,lst4=list(df.iloc[:, 1]),list(df.iloc[:, 2]),list(df.iloc[:, 3]),list(df.iloc[:, 4])
...
def shapelets_selection():
    ...
    X_train1, X_train2, X_train3, X_train4=store_csv_with_repo_list(repo_list_train, "index_train1")
